...
- **解析**：`YoutubeDL({'quiet': True, 'skip_download': True}).extract_info(url, download=False)`，从 `info['formats']` 中按 `height` 归并出 1080p/720p/480p/360p 档位；「最佳画质」档位通过 `_best_quality_label()` **标注具体分辨率**（如 `最佳画质 1080p`、`最佳画质 4K`），附加「仅音频 MP3」。
