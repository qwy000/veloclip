...
free-video-downloader/
  backend/
    app/
      ...
      api/routes.py           # 下载 API 路由
      api/ai_routes.py        # AI 分析 API
      services/downloader.py  # yt-dlp 封装(解析/下载/进度hook/ffmpeg定位)
      services/transcript.py  # 字幕提取(含 B 站弹幕兜底)
      services/llm.py         # DeepSeek API
      services/ai_analyzer.py # 总结/思维导图/问答编排
  ...
  frontend/                   # Vite+React+TS+Tailwind
    src/
      components/             # Downloader（含内嵌 AiAnalysisPanel）等
...
### 5.5 AI 分析 API（`/api/ai`，需 `DEEPSEEK_API_KEY`）

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/ai/analyze` | 提取字幕 + 生成总结与思维导图 Markdown |
| POST | `/api/ai/transcript` | 仅提取带时间戳字幕 |
| POST | `/api/ai/chat` | 基于字幕的多轮问答 |

配置见 `backend/.env.example`：`DEEPSEEK_API_KEY`、`DEEPSEEK_MODEL`（推荐 `deepseek-chat`）。

前端：`components/AiAnalyzer.tsx` 导出 `AiAnalysisPanel`，嵌入 `Downloader.tsx` 解析结果卡片；思维导图 PNG 导出使用离屏 DOM 克隆，避免 `transform: scale()` 导致 html-to-image 量算错误、节点边框压字。

## 6. 关键实现细节

### 6.1 yt-dlp 封装（services/downloader.py）
- **解析**：`YoutubeDL({'quiet': True, 'skip_download': True}).extract_info(url, download=False)`，从 `info['formats']` 中按**短边像素**（横屏取 height、竖屏取 width）归并 1080p/720p/480p 档位；「最佳画质」通过 `_best_quality_label()` 标注具体分辨率。
- **链接预处理**：`resolve_video_url()` 先 `normalize_url()`；若非平台直链则抓取页面 HTML，按正则提取嵌入的 B 站/YouTube/抖音等 URL 后再解析。`InfoResponse.resolved_url` 回传识别出的直链。
...
页面区块（营销 + 工具一体单页）：
1. 顶部导航（**VeloClip** Logo + 链接 + "升级 Pro" 蓝色胶囊 CTA）
...
- **AI 能力**：✅ 已实现 `/api/ai/*`（DeepSeek）；待扩展字幕翻译、批量分析队列。
...
## 12. 全选 / 定价布局 / 网页视频识别（已实现，2026-06-13 ~ 14）

> Cookie 网页上传方案**已取消**（见 12.4）；本节记录实际上线能力。

### 12.1 已实现项

| 项 | 说明 |
| --- | --- |
| 全选按钮 | `Downloader.tsx`：`inputRef` + 「全选」与「粘贴」并列 |
| 定价布局 | `App.tsx`：`max-w-6xl`、`sm:grid-cols-3` 横排紧凑、CTA → `#download` |
| 网页嵌入识别 | `downloader.py`：`resolve_video_url()` + `_extract_embedded_video_url()` |
| 品牌 VeloClip | 前端 `SITE_NAME`、标题 `VeloClip · 万能视频下载器` |
| 最佳画质文案 | `_best_quality_label()` 显示具体分辨率，非「（自动）」 |

### 12.2 网页视频识别流程

```mermaid
flowchart LR
  user["用户粘贴链接"] --> resolve["resolve_video_url"]
  resolve -->|"已是平台直链"| ytdlp["yt-dlp extract_info"]
  resolve -->|"课程/文章页"| fetch["抓取 HTML"]
  fetch --> extract["正则提取 B站/YouTube/抖音 URL"]
  extract --> normalize["normalize_url"]
  normalize --> ytdlp
  ytdlp --> ui["前端展示 resolved_url"]
```

**嵌入识别正则**（优先级）：B 站完整 URL / b23 短链 / YouTube / 抖音 / 西瓜 / TikTok / 裸 BV 号。

**典型用例**：编程导航课程页内嵌 B 站 BV，粘贴 section URL 即可解析。

### 12.3 最佳画质标签

- 从 `formats` 视频流取最高 `height`，映射到标准档位（4K / 1440p / 1080p …）
- 若无流高度，回退 `info['height']`
- 示例：`最佳画质 480p`、`最佳画质 1080p`、`最佳画质 4K`

### 12.4 Cookie 网页上传（已取消）

原方案含 `/api/cookies/*` + `CookiePanel.tsx`，因**用户不愿在网页配置 Cookie** 而取消。产品原则：**粘贴链接 → 解析 → 下载**。服务端仍支持 `backend/cookies/*.txt` 与 `YTDLP_COOKIES` 环境变量作运维兜底，对用户不可见。

### 12.5 测试与验收

- 脚本：`backend/scripts/test_url_resolve.py`（URL 规范化与嵌入识别）
- 构建：`npm run build`
- 浏览器：全选、定价三列、VeloClip 标题、解析后「最佳画质 xxxp」
- **部署注意**：修改后端后须重启 `uvicorn --reload`，否则 API 仍返回旧 label

### 12.6 涉及文件

| 文件 | 变更 |
| --- | --- |
| `backend/app/services/downloader.py` | `resolve_video_url`、`_best_quality_label`、iqiyi 映射 |
| `backend/app/models/schemas.py` | `resolved_url` 字段 |
| `frontend/src/components/Downloader.tsx` | 全选、resolved_url 展示 |
| `frontend/src/App.tsx` | VeloClip 品牌、Pricing 布局 |
| `frontend/index.html` | 页面标题 |
| `backend/scripts/test_url_resolve.py` | URL 回归测试 |

## 13. AI 字幕提取与 Errno 22 修复（2026-06-20）

### 13.1 问题现象

- B 站视频（如 `BV1awjw6qEog`）点击「开始 AI 分析」报 `[Errno 22] Invalid argument`
- 直接 `python` 调用 `extract_transcript()` 成功，经旧 uvicorn 进程访问 API 失败

### 13.2 根因

1. **字幕下载路径**：`transcript.py` 原先用 `_base_opts()`，内含 `ffmpeg_location`（imageio-ffmpeg 4.2.2）。部分 CC 字幕格式走 ffmpeg 后处理，Windows 下触发 `Invalid argument`。
2. **文件名**：未强制 `windowsfilenames` / 固定 outtmpl 时，标题含特殊字符可能导致写入失败。
3. **B 站策略**：CC 常需登录；弹幕免 Cookie 更稳，AI 分析应优先 danmaku。
4. **开发环境**：8000 端口被未退出的旧 uvicorn 占用，新代码未生效，请求仍打到旧进程。

### 13.3 方案（`transcript.py`）

```text
_subtitle_opts(url, tmp)
  ├─ 移除 ffmpeg_location（仅拉字幕，不转码）
  ├─ skip_download: True
  ├─ outtmpl: { default/subtitle → tmp/video }  # 固定短路径
  └─ windowsfilenames + restrictfilenames + trim_file_name

_pick_subtitle_lang(..., video_url)
  └─ B 站且存在 danmaku → 优先弹幕

_download_subtitle_files(...)
  └─ CC 失败 → 回退 danmaku XML
```

`ai_routes._friendly_ai_error()` 将 errno 22 映射为中文提示。

### 13.4 抖音字幕说明

公开 API（`aweme/v1/web/aweme/detail/`）在无登录 Cookie 时不返回 `cla_info` / `subtitleInfos` / `interaction_stickers`。播放器内可见字幕多为**硬字幕**或需浏览器会话加载的软字幕，当前 yt-dlp 链路无法提取。

### 13.5 运维注意

- 修改 `transcript.py` 后**必须重启 uvicorn**；若绑定 8000 失败，在任务管理器结束占用端口的 python 进程。
- 验收：`POST /api/ai/analyze` + `https://www.bilibili.com/video/BV1awjw6qEog` 应返回总结与字幕列表。

## 14. 字幕写入容错、无字幕降级与开发端口（2026-06-22）

### 14.1 AI 分析字幕链（更新）

```text
extract_for_analysis(url)          # AI 分析 / Chat
  → yt-dlp 探测 + 下载字幕
  → CC / auto / danmaku 解析
  └─ 失败且无字幕（除抖音）
       → _metadata_transcript_result（title/description/uploader/tags）
       → subtitle_source = metadata

extract_transcript(url)            # /api/ai/transcript 严格模式，无字幕仍 422
```

B 站 `_pick_subtitle_lang`：**CC 优先**，下载失败时 `_download_subtitle_files` 回退 danmaku。

### 14.2 字幕写入容错

- `_ensure_subtitle_dir` / `_subtitle_stem`：固定 `{tmpdir}/video`，避免标题特殊字符
- `_collect_subtitle_files`：过滤空文件；支持 `.vtt/.srt/.xml` 等
- 弹幕 XML / 字幕解析异常：日志 + 空列表，不崩溃
- 读取统一 `encoding=utf-8, errors=replace`

### 14.3 无字幕 AI Prompt

`ai_analyzer.py` 对 `source=metadata` 使用专用 SUMMARY/MINDMAP/CHAT prompt，明确标注「未检测到字幕」。

### 14.4 开发与运维

- 前端 Vite 代理 **8001**（Windows 8000 僵尸进程规避）
- `GET /health` → `{ status, pid }`
- 单元测试：`backend/tests/test_transcript.py`
- 思维导图：`MindMapView` 全屏（Fullscreen API）

### 14.5 平台差异

| 平台 | 无字幕行为 |
| --- | --- |
| X / Twitter | 元数据降级，可选 `cookies/twitter.txt` |
| B 站 | CC → 弹幕；均无则 metadata |
| 抖音 | 仍 422（硬字幕） |
| YouTube 等 | CC/auto 或 metadata |

## 15. 定价区紧凑布局与 CTA 跳转（2026-06-22）

### 15.1 布局

- `Pricing`：`max-w-6xl`、`sm:grid-cols-3`、`gap-3/4`、卡片 `p-4 sm:p-5`
- 价格字号 `text-2xl sm:text-3xl`，特性列表 `text-xs sm:text-sm`
- 按钮改为 `<a href="#download">`，与底部 CTA 一致

### 15.2 Hero 锚点

- Hero 区块增加 `id="download"`、`scroll-mt-16`（配合 sticky 导航）
- 底部「立即免费下载」由 `#top` 改为 `#download`

### 15.3 文案

- 避免「保证无水印」表述；FAQ / Feature 按平台实际能力描述
- README 补充仓库地址、测试命令、AI 与 Cookie 说明
